...
```bash
# 1. 创建网络
docker network create sturegsys-network

# 2. 启动数据库（MariaDB）
docker run -d \
  --network sturegsys-network \
  --restart always \
  --name sturegsys-mysql \
  -e MYSQL_ROOT_PASSWORD=123456 \
  -e MYSQL_DATABASE=sturegsys \
  -p 3306:3306 \
  mariadb:10.11

# 3. 初始化数据库（请确保 data.sql 文件在当前目录）
docker exec -i sturegsys-mysql mysql -uroot -p123456 < data.sql

# 4. 启动后端
docker run -d --name sturegsys-backend --network sturegsys-network --restart always \
  -p 8081:8081 -e SPRING_DATASOURCE_URL=jdbc:mysql://sturegsys-mysql:3306/sturegsys \
  -e SPRING_DATASOURCE_USERNAME=root -e SPRING_DATASOURCE_PASSWORD=123456 \
  xuemuluo/sturegsys:backend-v1

# 5. 启动前端
docker run -d --name sturegsys-frontend --network sturegsys-network --restart always \
  -p 80:80 xuemuluo/sturegsys:frontend-v1
```
...
# 查看容器日志
docker logs sturegsys-backend
docker logs sturegsys-frontend
docker logs sturegsys-mysql

# 停止容器
docker stop sturegsys-backend sturegsys-frontend sturegsys-mysql

# 启动容器
docker start sturegsys-mysql sturegsys-backend sturegsys-frontend

# 删除容器
docker rm -f sturegsys-backend sturegsys-frontend sturegsys-mysql
```
