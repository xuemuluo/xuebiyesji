...
将 rose-three.html 中的玫瑰曲线（Rose Three）数学动画集成到项目开头作为加载动画，替换原有的 Element Plus 加载效果。

### 修改内容

#### 1. RoseLoader.vue - 新增

**更改原因：** 创建独立的玫瑰曲线加载动画组件

**更改方式：**
- 封装 rose-three.html 中的 SVG 玫瑰曲线动画（r = a cos(3θ)）
- 使用 76 个粒子沿曲线轨迹运动，带旋转和呼吸脉动效果
- 添加淡出过渡动画（0.8秒，带轻微缩放效果）
- 白色背景 + 深灰色动画 + 黑色文字配色方案
- 动画尺寸缩小至 200px，标题 18px，副标题 12px

#### 2. App.vue - 修改

**更改原因：** 集成玫瑰曲线加载动画替代原有加载效果

**更改方式：**
- 引入 RoseLoader 组件
- 移除 Element Plus v-loading 指令
- 使用 v-show 控制主内容显示
- 加载完成后延迟 1.5 秒显示主内容，让用户看到动画
